...
const WorkerAddSheet = props => {
  ...
  const _onOpen = async () => {
    setLoading(true);
    try {
      const {data, status} = await services.getOffEmployees(props.payload.date);
      const allEmployees = await services.getEmployees();
      allEmployees.data.map(emp => {
        if (employees.includes(emp.id)) {
          data.push({...emp, selected: true});
        }
      });
      ...
      if (status === 200) {
        ...
        setJSON(() => {
          const result = data.filter(item => {
            item.selected = employees.includes(item.id);
            return item;
          });
          return [...result];
        });
