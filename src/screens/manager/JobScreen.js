...
const JobScreen = ({navigation, route}) => {
  ...
  const _fetchData = async () => {
    ...
    try {
      ...
      const employeeIds = data.ProjectUsers.map(employee => employee.User.id);
  ...
  const _addWorker = async () => {
    const {added, removed} = await SheetManager.show('WorkerAddSheet', {
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      <View
        ...
        />
        <Text style={{fontWeight: 'bold', marginLeft: 10}}>{project.name}</Text>
      </View>
      <View
        style={{
