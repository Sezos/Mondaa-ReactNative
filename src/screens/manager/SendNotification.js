...
export default function SendNotification({navigation}) {
  ...
  const _addWorker = async () => {
    ...
    if (added.length !== 0) {
      setSelectedUsers(selectedUsers.concat(added));
    }
    if (removed.length !== 0) {
      setSelectedUsers(selectedUsers.filter(user => !removed.includes(user)));
    }
  };
  const send = async () => {
    try {
                  ...
                  if (selectedUsers.includes(user.id)) {
                    return (
                      <DataTable.Row key={idx}>
                        <DataTable.Cell>{user.employeeId}</DataTable.Cell>
                        <DataTable.Cell>
